...
import type { ScenarioCumulativeResponse, ScenarioTimeseriesResponse } from '../api/types';
import ScenarioComparisonPage from './ScenarioComparisonPage';

vi.mock('../api/client', () => ({
  api: { scenarioTimeseries: vi.fn(), scenarioCumulative: vi.fn() },
}));
...
describe('ScenarioComparisonPage', () => {
  it('defaults to Single Country view for the first country', async () => {
  ...
  it('switches to Global Aggregate and drops the country param', async () => {
    ...
    vi.mocked(api.scenarioTimeseries).mockResolvedValue({ ...TIMESERIES, title_suffix: 'All 10 Countries' });
    await user.click(screen.getByText('Global Aggregate'));

    expect(await screen.findByText('CO₂ Emissions Scenarios — All 10 Countries')).toBeInTheDocument();

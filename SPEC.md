...
| Backward compatibility | The dashboard client (`climate-dashboard-react/src/api/client.ts`) sends only `countries`/`gas` to these endpoints, never `scope` — confirmed by direct inspection before shipping. With `scope` defaulting to `"expanded"`, every existing dashboard call resolves identically to before. `get_timeseries`'s no-`countries` fallback (originally `FEATURED_COUNTRIES[:5]`, widened to the full `FEATURED_COUNTRIES` by PR #134 below) stays scope-independent regardless of list size; `get_decade_composition`'s no-`countries` fallback (the whole pool) now generalizes to "the whole *selected-scope* pool," a direct extension of its prior "whole expanded pool" behavior. |
| Follow-up (PR #134, Shipped) | `get_timeseries`'s no-`countries` default widened from `FEATURED_COUNTRIES[:5]` (5 countries, a straggler from before the featured list grew to 10) to the full `FEATURED_COUNTRIES` (10), matching the React frontend's own picker default. Also adds a pinning test for the pre-existing (not new) fact above — `scope` has no observable effect on this endpoint's no-`countries` default at any list size, unlike `get_decade_composition`'s. Confirmed via `mcp-server-spec.md` §3.2 that no real MCP-layer caller hits this combination, so no code change to `scope`'s behavior was needed, only the default list size and a test locking in the existing scope-blind fact. |
...
### 5.23 Per-Capita / YoY Growth / Per-GDP Fields on `GET /historical/timeseries` (Shipped, `climate-emissions-analysis-project` PR #139)

**Status: Shipped.** Another mentor addition outside the internship curriculum, same category as

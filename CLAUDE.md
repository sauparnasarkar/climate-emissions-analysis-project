...
> are actually asked to build. `api/`, `climate-dashboard-react/`, `services/mcp-server/`, and
> `services/agent/` are a **separate, later expansion built by the mentor**, turning this
> project into a reference example of a production-shaped data engineering + front-end
> dashboard + agent-tooling stack — **not** an internship deliverable, not graded, not part of
> intern certification. See `SPEC.md` §5 for the `api/`/`climate-dashboard-react/` addendum,
> [`services/mcp-server/SPEC.md`](services/mcp-server/SPEC.md) for the MCP server's own spec
> (Stage 1 of the conversational-agent project), and
> [`services/agent/SPEC.md`](services/agent/SPEC.md) for the LangGraph agent built on top of it
> (Stage 2) — all documented separately from the internship spec for exactly this reason.
...
- `services/agent/` — LangGraph conversational agent (mentor's post-internship reference-architecture expansion, not internship scope), surfaced as a new nav item in `climate-dashboard-react/` — Stage 2 of the conversational-agent project `services/mcp-server` began. Calls `services/mcp-server`'s tools as an MCP client (`langchain-mcp-adapters`), unauthenticated over localhost since it's co-located with `services/mcp-server` on the same deploy host; renders results as real `design-system` components, never generated markup. Independently versioned/deployable (own `pyproject.toml`). Also hosts this repo's first admin-panel capability (LLM provider/model switching) — see the "Admin capabilities" note below. Own [`CLAUDE.md`](services/agent/CLAUDE.md) and [`SPEC.md`](services/agent/SPEC.md) — see those rather than this file for its design and node catalog
...
**Admin capabilities (cross-cutting).** A single Cloudflare Access-gated admin surface (login
policy, Google as identity provider, one allow-listed account) spans multiple sub-projects rather
than living inside any one of them — see [`ARCHITECTURE.md`](ARCHITECTURE.md) §8 for the shared
Access-application/path-rule convention and the "each service owns its own admin routes, no
shared admin service" rule. Today's only capability, LLM provider/model switching, lives entirely
in `services/agent` — see its own [`SPEC.md`](services/agent/SPEC.md).
...
- **Two-tier country pattern (Release 2.1, `ENHANCEMENTS.md`):** `FEATURED_COUNTRIES` (10, hardcoded — China, USA, India, Russia, Japan, Germany, Brazil, UK, South Africa, Australia) is the original curated set, still used as the default/narrative selection everywhere (fixed-size 5×2 subplot grids, seeded dropdown defaults). A `get_expanded_countries()` function — defined independently in `notebook/constants.py` and `app.py`, and as `load_expanded_countries()` in `api/data_loaders.py` — loads a data-driven set of ~40 major emitters (coverage + materiality thresholds, computed in Week 1 §1.2, persisted to `data/selected_countries.json`), used everywhere else: per-country training/evaluation loops, aggregate sums, tables, and any interactive picker's searchable pool. The notebook version raises `FileNotFoundError` uncaught (Weeks 2–5 can't proceed without it); the `app.py`/`api` versions degrade gracefully, falling back to `FEATURED_COUNTRIES` with a warning instead of crashing. Multi-country pickers that feed a chart (React `MultiSelect`, Streamlit `st.multiselect`) cap simultaneous selections at 10 even though their searchable pool is the full expanded list — past 10 countries on one chart stops being readable. **The Overview page (Release 2.2, `SPEC.md` §5.7) extends this into three simultaneous tiers** instead of picking one: "All Countries" (every sovereign country, `NON_SOVEREIGN` excluded, unfiltered by coverage/Mt), "Expanded" (the ~40-country set above), and "Selected" (a user-picked subset capped at 10, defaulted to `FEATURED_COUNTRIES`) — only the Selected tier drives the bar chart/% change chart/Top Movers.
...
- **Conversational agent specification:** [`services/agent/SPEC.md`](services/agent/SPEC.md) — the LangGraph node catalog, UI-intent schema, guardrails, and generative-UI design for the Stage 2 agent sub-project; kept separate from the root `SPEC.md` for the same reason as the MCP server's own spec. Companion [`services/agent/CLAUDE.md`](services/agent/CLAUDE.md) and its own `ENHANCEMENTS.md`/`ARCHITECTURE.md` cover that sub-project's day-to-day guidance, release history, and cross-cutting architecture
- **Technical architecture:** [`ARCHITECTURE.md`](ARCHITECTURE.md) — a current-state description of how the system is actually built (data pipeline, `api`/`climate-dashboard-react`/`services/mcp-server`/`services/agent` internals, deploy topology on the Mac Mini, design-system integration). Unlike `SPEC.md`/`ENHANCEMENTS.md`, this doesn't narrate history — update it only on architecturally-significant changes (new service, new data flow, new pattern), not per bug fix
